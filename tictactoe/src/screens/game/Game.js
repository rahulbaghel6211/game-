...
/>
    ...
    movePieceByPosition(piece, move){
        let newPosition = "";
        const position = parseInt(piece.position.substring(1, piece.position.length));
        const cellAreaIndicator = piece.position.substring(0, 1);
    ...
    onPieceSelection = (selectedPiece) => {
        ...
        const { moves, turn } = this.state;
        const player = this.state[selectedPiece.color];
      
        if (moves.length === 1) {
          // Only one move left in the current turn
          if (selectedPiece.position === HOME && moves[0] !== 6) {
            // You can only move a piece out of HOME with a 6
            return;
          }
          this.movePieceByPosition(selectedPiece, moves.shift());
        } else if (moves.length > 1) {
          // Multiple moves left in the current turn
          if (selectedPiece.position === HOME) {
            // The selected piece is in HOME
            if (moves.includes(6)) {
              // If there's a 6, use it to move the piece out of HOME
              moves.shift();
              ...
              this.setState(this.state, () => {
                if (moves.length === 1 && !this.playerHasOptionsForMoves(player)) {
                  // Only one move left and no other options, make the move
                  this.movePieceByPosition(selectedPiece, moves.shift());
                }
              });
            } else {
              // If no 6, you can't move a piece from HOME
              return;
            }
          } else {
            // The selected piece is not in HOME
            ...
            // Display move options for the player to choose from
